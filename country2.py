...
# 구글 드라이버 위치
s = Service("E:/work/pythonCrawling/chromedriver.exe")
...
# 엑셀 파일로 데이터 만들기
def makeExcel(data, sheetname, filename):
    for name in sheetname:
        dataframe = pd.DataFrame(data, columns=["업무", "공고번호-차수", "URL-확인", "분류" ,"공고명", "URL", "공고기관", "수요기관", "계약방법", "입력일시\n(입찰마감일시)", "공동수급", "투찰"]) # columns 설정
        dataframe = dataframe.drop('URL-확인', axis='columns') # URL-확인 열 삭제
        # Excel 파일 생성
        wb = Workbook()
        ws = wb.active
        ws.title = name[0]
        # DataFrame의 내용을 Excel 파일로 옮기기
        for r_idx, row in enumerate(dataframe_to_rows(data, index=1, header=True)):
...
if __name__ == "__main__":
    crawling = country(['기상', '환경'])
    makeExcel(crawling, ['기상', '환경'], '나라장터')